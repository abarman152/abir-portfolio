...
| Method | Endpoint | Auth | Description |
|---|---|---|---|
| `GET` | `/admin/dashboard` | [AUTH] | Aggregate counts (projects, research, certifications, achievements, unread messages) for the admin dashboard overview |

Mounted at `/api/admin/dashboard` in `backend/src/index.ts` — note the `/admin` prefix, which is easy to miss since every other authenticated route is mounted directly under `/api/<resource>` without an `/admin` segment. See [`../features/dashboard-analytics.md`](../features/dashboard-analytics.md).
