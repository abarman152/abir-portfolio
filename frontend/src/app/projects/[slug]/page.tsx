      ...
      <main style={{ minHeight: '100vh', background: 'var(--bg)', paddingTop: '60px' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto', padding: '2.5rem 2rem 4rem' }}>
          ...
          <Link
            ...
            onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.borderColor = 'var(--border-2)'; e.currentTarget.style.color = 'var(--text)'; }}
            onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-2)'; }}
          >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1.25rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.625rem', flexShrink: 0 }}>
              {project.githubUrl && (
                <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1.1rem', borderRadius: '9px', border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text)', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600, transition: 'border-color 0.15s' }} onMouseEnter={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.borderColor = 'var(--border-2)')} onMouseLeave={(e: React.MouseEvent<HTMLAnchorElement>) => (e.currentTarget.style.borderColor = 'var(--border)')}>
                  <GitFork size={14} /> Code
                </a>
              )}
              {project.liveUrl && (
                <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 1.1rem', borderRadius: '9px', border: 'none', background: 'var(--accent)', color: 'white', textDecoration: 'none', fontSize: '0.85rem', fontWeight: 600 }}>
          ...
          {(project.problem || project.result) && (
            <div style={{ display: 'grid', gridTemplateColumns: project.problem && project.result ? '1fr 1fr' : '1fr', gap: '1rem', marginBottom: '2rem' }} className="detail-pr-grid">
              {project.problem && (
                <div className="card" style={{ padding: '1.5rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.875rem' }}>
                    <span style={{ fontSize: '0.63rem', fontWeight: 800, color: '#ef4444', background: '#ef444412', padding: '0.18rem 0.45rem', borderRadius: '4px', letterSpacing: '0.07em' }}>PROBLEM</span>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {project.imageUrl ? (
                    <div className="card" style={{ padding: '1.5rem', flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.875rem' }}>
                        <span style={{ fontSize: '0.63rem', fontWeight: 800, color: '#10b981', background: '#10b98112', padding: '0.18rem 0.45rem', borderRadius: '4px', letterSpacing: '0.07em' }}>RESULT</span>
                      </div>
