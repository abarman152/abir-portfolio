...
export default function AboutPageContent({ profile, education, skillGroups, achievements }: Props) {
  const contacts = [
    ...
    { icon: LinkedInIcon,text: (profile.linkedinUrl || '').replace('https://www.', '').replace('https://', ''), href: profile.linkedinUrl },
    { icon: GitHubIcon,  text: (profile.githubUrl   || '').replace('https://www.', '').replace('https://', ''), href: profile.githubUrl },
